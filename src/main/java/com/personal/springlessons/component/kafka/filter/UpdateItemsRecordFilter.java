...
@Slf4j
@Component(value = "updateItemsFilter")
public class UpdateItemsRecordFilter<K, V> implements RecordFilterStrategy<K, V> {
    ...
    public boolean filter(ConsumerRecord<K, V> consumerRecord) {
        ...
        if (null != consumerRecord) {
            ...
            if (!ItemStatus.UPDATE.equals(message.getItemStatus())) {
                result = true;
                log.debug(
                    "Filter for item status: 'UPDATE', item status received: '{}' message filtered on topic: '{}', offset: '{}', partition: '{}'",
