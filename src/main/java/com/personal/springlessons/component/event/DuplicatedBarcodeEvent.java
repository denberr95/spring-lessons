...
public class DuplicatedBarcodeEvent {
    ...
    public void duplicatedBarcodeHandler(KafkaMessageItemDTO kafkaMessageItemDTO) {
        ...
        DiscardedItem discardedItem = this.itemsMapper.mapCsv(kafkaMessageItemDTO);
        String file = this.getFullFileName(discardedItem);

        try (FileWriter writer = new FileWriter(file);) {
            ColumnPositionMappingStrategy<DiscardedItem> strategy =
                    new ColumnPositionMappingStrategy<>();
            strategy.setType(DiscardedItem.class);
            StatefulBeanToCsv<DiscardedItem> beanToCsv =
                    new StatefulBeanToCsvBuilder<DiscardedItem>(writer)
    ...
    private String getFullFileName(DiscardedItem discardedItem) {
